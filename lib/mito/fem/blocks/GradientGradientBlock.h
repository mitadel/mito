    ...
    class GradientGradientBlock {
      ...
      public:
        ...
        auto compute(const elementT & element) const -> elementary_shape
        {
            ...
            // the ambient metric field
            const auto g_ambient = elementT::ambient_metric_type::field();
            ...
            tensor::constexpr_for_1<n_quads>([&]<int q>() {
                ...
                // the pullback metric: g = J^T * G(x) * J
                const auto G = g_ambient(x);
                const auto g = tensor::transpose(J) * G * J;
                const auto ginv = tensor::inverse(g);
                const auto dV = w(x)(tensor::columns(J));
                const auto coefficient = _coefficient(x);

                // the shape function gradients at this quadrature point
                const auto dphi = [&]<int... a>(tensor::integer_sequence<a...>) {
                    return std::array{ (
                        J * (ginv * elementT::shape_functions.template dshape<a>()(xi)))... };
                ...
                tensor::constexpr_for_1<n_nodes>([&]<int a>() {
                    tensor::constexpr_for_1<n_nodes>([&]<int b>() {
                        elementary_matrix[{ a, b }] +=
                            w_q * dV * (dphi[a] * (coefficient * dphi[b]));
                    });
                });
            });
