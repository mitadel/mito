...
InputFile::InputFile(std::string inputFileName) : _dictionaryReals(), _dictionaryStrings()
{
    if (inputFileName == "")
        return;
...
InputFile::InputFile(int argc, char ** argv) : InputFile(getFileName(argc, argv)) {}

void
InputFile::Display() const
{
    // report
...
int
InputFile::_readFile(std::string inputFileName)
{
    // try to open file
    ...
    std::string number_expression(
        "-?(?:[0-9]+|[0-9]*.[0-9]+|[0-9]+.[0-9]*)" + e_number_expression + "?");
