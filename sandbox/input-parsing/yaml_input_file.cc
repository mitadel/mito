...
YAMLInputFile::YAMLInputFile(int argc, char ** argv) : YAMLInputFile(getFileName(argc, argv)) {}

int
YAMLInputFile::_readFile(std::string inputFileName)
{
...
YAMLInputFile::Display() const
{
    ...
    for (YAML::const_iterator it = std::begin(_file); it != std::end(_file); ++it) {
        ...
        // report
        std::cout << "\t" << it->first.as<std::string>() << ":\t\t" << it->second.as<real>()
                  << std::endl;
    }
