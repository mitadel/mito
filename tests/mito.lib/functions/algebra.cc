...
TEST(Algebra, ScalarValuedFunctions)
{
    ...
    constexpr mito::tensor::vector_t<3> my_vector = { 1, 2, 3 };
...
TEST(Algebra, TensorValuedFunctions)
{
    ...
    // a point in space
    constexpr auto x = 2.0 * mito::tensor::e_1<3> + mito::tensor::e_2<3>;
    ...
    constexpr auto B1 = f * mito::tensor::e_12<3> + g * mito::tensor::e_22<3>;

    // a tensor function
    constexpr auto B2 = mito::functions::constant<mito::tensor::vector_t<3>>(mito::tensor::e_12<3>);

    // add the two tensor functions
    constexpr auto B = B1 + B2;

    // the tensor: (f(x) + 1) * e_12 + g(x) * e_21
    constexpr auto A = 3.0 * mito::tensor::e_12<3> + 4.0 * mito::tensor::e_22<3>;
