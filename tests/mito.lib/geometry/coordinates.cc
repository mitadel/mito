...
TEST(Coordinates, Euclidean)
{
    ...
    constexpr auto coord_A =
        mito::geometry::Coordinates<2, mito::geometry::EUCLIDEAN>({ 0.0, 1.0 });
    constexpr auto coord_B =
        mito::geometry::Coordinates<2, mito::geometry::EUCLIDEAN>({ 1.0, 0.0 });

    // the exact value of {coord_A + coord_B} / 2
    constexpr auto coord_C =
        mito::geometry::Coordinates<2, mito::geometry::EUCLIDEAN>({ 0.5, 0.5 });
...
{
    // create two sets of coordinates
    constexpr auto coord_A = mito::geometry::Coordinates<2, mito::geometry::POLAR>({ 1.0, 0.0 });
    constexpr auto coord_B =
        mito::geometry::Coordinates<2, mito::geometry::POLAR>({ 1.0, 0.5 * std::numbers::pi });

    // the exact value of {coord_A + coord_B} / 2.0
    constexpr auto coord_C = mito::geometry::Coordinates<2, mito::geometry::POLAR>(
