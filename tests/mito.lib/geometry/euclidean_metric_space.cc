...
TEST(Manifolds, EuclideanMetricSpace)
{
    ...
    static_assert(e_0(x) == mito::tensor::vector_t<2>({ 1.0, 0.0 }));

    constexpr auto e_1 = metric_space_t::e<1>;
    // check that it is equal to [0.0, 1.0] at point {x}
    static_assert(e_1(x) == mito::tensor::vector_t<2>({ 0.0, 1.0 }));
    ...
    constexpr auto dx0_scalar = mito::tensor::scalar_t{ 0.01 };
    constexpr auto dx1_scalar = mito::tensor::scalar_t{ 0.01 };
