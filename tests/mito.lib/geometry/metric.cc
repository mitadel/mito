...
{
    // pick a radius
    constexpr auto r = mito::scalar_t{ 2.0 };
    ...
    static_assert(polar_metric(point) == mito::diagonal_matrix_t<2>({ 1.0, r * r }));
}
...
TEST(Metric, Spherical)
{
    ...
    constexpr auto r = mito::scalar_t{ 2.0 };
    // and an angle
    constexpr auto theta = mito::scalar_t{ 1.5 };
    ...
    static_assert(
        ...
        == mito::diagonal_matrix_t<3>({ 1.0, r * r, r * r * std::sin(theta) * std::sin(theta) }));
}
