...
TEST(Manifolds, EuclideanSubmanifoldMetric3D)
{
    // the basis one-forms
    ...
    constexpr auto normal_form = mito::manifolds::one_form(normal_vector);

    // rebuild the volume form as wV = wedge(normal_form, wS)
    constexpr auto wV = mito::manifolds::wedge(normal_form, wS);
