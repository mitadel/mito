...
{
    // a vector in space
    constexpr mito::vector_t<2> x = { 0.0, 0.0 };

    // a scalar field
    constexpr auto f = mito::manifolds::field(
        [](const mito::vector_t<2> & x) -> mito::real { return cos(x[0] * x[1]); });

    // check value of field at {x}
    static_assert(f(x) == 1.0);

    // a vector field
    constexpr auto g = mito::manifolds::field([](const mito::vector_t<2> & x) -> mito::vector_t<2> {
