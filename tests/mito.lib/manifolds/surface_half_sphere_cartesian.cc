...
TEST(Manifolds, Sphere)
{
    ...
    constexpr auto normal_field = mito::manifolds::field([](const coordinates_t & x) -> auto {
    ...
    // the basis one-forms
    constexpr auto dx = mito::manifolds::one_form(
        e_x, mito::manifolds::identity_tensor_field<3, 3, mito::geometry::CARTESIAN>);
    constexpr auto dy = mito::manifolds::one_form(
        e_y, mito::manifolds::identity_tensor_field<3, 3, mito::geometry::CARTESIAN>);
    constexpr auto dz = mito::manifolds::one_form(
        e_z, mito::manifolds::identity_tensor_field<3, 3, mito::geometry::CARTESIAN>);
