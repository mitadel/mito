...
TEST(Identities, DivGrad)
{
    ...
    constexpr auto div_grad_T =
        mito::operators::divergence(mito::functions::transpose(mito::operators::gradient(f)));

    // the gradient of the divergence of {f}
    constexpr auto grad_div = mito::operators::gradient(mito::operators::divergence(f));
