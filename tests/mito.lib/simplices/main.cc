...
main()
{
    ...
    */
    ...
    assert(!mito::topology::exists_flipped(segment0));
    // assert no simplex rides on this segment yet
    assert(mito::topology::incidence(segment0) == 1);
    ...
    assert(!mito::topology::exists_flipped(segment1));
    // assert no simplex rides on this segment yet
    assert(mito::topology::incidence(segment1) == 1);
    ...
    assert(mito::topology::exists_flipped(segment1) && mito::topology::exists_flipped(segment1m));
    // assert no simplex rides on this segment yet
    assert(mito::topology::incidence(segment1) == 1);
    // assert no simplex rides on this segment yet
    assert(mito::topology::incidence(segment1m) == 1);
    ...
    assert(!mito::topology::exists_flipped(segment2));
    // assert no simplex rides on this segment yet
    assert(mito::topology::incidence(segment2) == 1);
    ...
    assert(!mito::topology::exists_flipped(segment3));
    // assert no simplex rides on this segment yet
    assert(mito::topology::incidence(segment3) == 1);
    ...
    assert(!mito::topology::exists_flipped(segment4));
    // assert no simplex rides on this segment yet
    assert(mito::topology::incidence(segment4) == 1);
    ...
    // assert there is one simplex riding on this segment (element0)
    assert(mito::topology::incidence(segment1) == 2);
    // assert no simplex rides on this segment yet
    assert(mito::topology::incidence(segment1m) == 1);
    // assert there is one simplex riding on this segment (element0)
    assert(mito::topology::incidence(segment2) == 2);

    // assert there is one triangle connecting {segment0}, {segment1}, and {segment2}
    assert(!mito::topology::exists_flipped(element0));
    // assert no simplex rides on this segment
    assert(mito::topology::incidence(element0) == 1);
    ...
    // assert there is one simplex riding on this segment (element1)
    assert(mito::topology::incidence(segment4) == 2);
    // assert there is one simplex riding on this segment (element1)
    assert(mito::topology::incidence(segment1m) == 2);

    // assert there is one triangle connecting {segment3}, {segment4}, and {segment1m}
    assert(!mito::topology::exists_flipped(element1));
    // assert no simplex rides on this segment
